...
export const createInvitation = (inviterId, inviteeId, groupId) => {
    return new Promise((resolve, reject) => {
        db.query(`INSERT INTO invitaion (inviter, invitee, group_id, status)
            VALUES (?, ?, ?, #)`, [inviterId, inviteeId, groupId], (error, results) => {
            if (error) {
                reject(error);
            } else {
                resolve(results);
...
export const getOverviewByGroupId = (groupId) => {
    return new Promise((resolve, reject) => {
        db.query(`SELECT tg.group_name, tg.start_date, tg.end_date, tg.status, u.user_name
                  FROM trip_groups tg
                  JOIN group_member gm ON tg.group_id = gm.g_id
                  JOIN user_account u ON gm.u_id = u.user_id
                  WHERE tg.group_id = ?`, [groupId], (error, results) => {
            if (error) {
                reject(error);
            } else {
                resolve(results);
...
export const getInvitationByUserId = (userId) => {
    return new Promise((resolve, reject) => {
        db.query(`SELECT u.user_name AS inviter_name, tg.group_name AS group_name, i.status
            FROM invitation i
            JOIN user_account u ON i.inviter = u.user_id
            JOIN trip_groups tg ON i.group_id = tg.group_id
            WHERE i.invitee = ?`, [userId], (error, results) => {
            if (error) {
                reject(error);
            } else {
                resolve(results);
...
export const updateInvitation = (invitationId, status) => {
    return new Promise((resolve, reject) => {
        db.beginTransaction((err) => {
            ...
            db.query(`UPDATE invitation SET status = ? WHERE invitation_id = ?`, [status, invitationId], (error, results) => {
                if (error) {
                    db.rollback(() => {
                        reject(error);
                    });
                    ...
                    if (status === "Y") {
                        db.query(`SELECT group_id, invitee_id FROM invitation WHERE invitation_id = ?`, [invitationId], (error, results) => {
                            if (error) {
                                db.rollback(() => {
                                    reject(error);
                                });
                                ...
                                db.query(`INSERT INTO group_member (u_id, g_id) VALUES (?, ?)`, [inviteeId, groupId], (error, results) => {
                                    if (error) {
                                        db.rollback(() => {
                                            reject(error);
                                        });
